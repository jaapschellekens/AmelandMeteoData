...
rep = ts_range(a_f,sprintf('01/%d/%d 00:00:00',month,year),\
...
fprintf(stderr,'Making daily values...\r');
secs = [dd(1); dd] * 86400;
msecs = ts_reduce([rep(:,1) secs],1,1,1);
msecs = msecs(:,2);
...
fprintf(of,'\\section{Daily values in month %d (%d)}\n\n',month,year);

fprintf(of,'\\footnotesize\\begin{verbatim}');

dd = ts_reduce(rep,1,1,1);
ddd = ts_reduce(rep,0,1,1);

dr = [ddd(:,1) ddd(:,4) dd(:,[11 7 5 9 14])]; 

dmax = ts_reduce(rep,2,1,1);
dmin = ts_reduce(rep,3,1,1);
...
system(sprintf('dvips -o %smonth.ps %d_%d.dvi > /dev/null ',off,month,year));
